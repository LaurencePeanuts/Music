...
\ee
We make this choice of parametrization for the first element of $\vec{\eta}$ because we expect the amplitude of the power spectrum to be well-constrained by the data $a_{lm}$, whereas $H_*$ and $V(\varphi_*)$, while degenerate with $\epsilon_*$, are easily obtained from the $\mathcal{P}_\zeta(k_*)$ amplitude once a value of $\epsilon_*$ is picked:
\bea
	\frac{H^2_*}{\epsilon_*}=8\pi^2\Mp^2\mathcal{P}_\zeta(k_*)\quad&\Rightarrow&\quad H_*=\sqrt{\mathcal{P}_\zeta(k_*)\, 8\pi^2\epsilon_*(2.435\times10^{18}\mathrm{GeV})^2}\, ,\\
...
%\section{Calculating the minimum value of $\epsilon_*$}
%
%\bea
%	\epsilon_*=\epsilon_{C*}+\epsilon_{\xi*}&= &\frac{\dot\varphi^2}{2\Mp^2H^2}+\frac{\dot\varphi}{2\pi}\tilde{\xi}\, , \nonumber \\
%		&=& 
%\eea
