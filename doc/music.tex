...
\begin{itemize}
\item Represent the big bang as a sphere of comoving radius $\sim14.23$~Gpc (adopting a Hubble constant of 68 km s$^{-1}$ Mpc$^{-1}$).  The last scattering surface -- the \emph{cosmic photosphere} --  is a sphere with radius $\sim0.29$~Gpc smaller. Recombination occurs over a short but finite interval of time just prior to last scattering. The radius of the cosmic photosphere, 13.94~Gpc, is our unit of length.
\item Ignore the expansion of the universe and just consider the potential at the time of last scattering when all modes of interest can be considered as linear. These can be evolved backward and forward in time with confidence. In practice, the potential changes little after recombination although shorter-wavelength modes eventually become nonlinear. (This, too, can be accommodated in principle.) Our potential can therefor be expanded as:
...
\item Presume that the potential within our horizon can be expanded as a set of discrete Fourier components defined within a box of side 4 with periodic boundary conditions and fundamental wavenumber $\pi/2$. We truncate at finite maximum wavenumber. Shorter wavelengths contribute an effective measurement error. 
