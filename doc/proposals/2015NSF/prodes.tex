...
\begin{equation}
-2\ln{\cal P} \approx (a_y- \mathbf{R}_{yn}f_n)^{\mathrm{T}}C_{yy'}^{-1}(a_{y'}-\mathbf{R}_{y'n'}f_n')+\frac{f_n^2}{\sigma_{\bf n}^2} + \mathrm{const.}
\end{equation}
...
In Section~\ref{} below, we give a more rigorously-derived expression for the posterior PDF ${\cal P}$. The approximate form given here is sufficient for a vital early feasibility check:
