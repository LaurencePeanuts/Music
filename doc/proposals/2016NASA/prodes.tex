...
surveys, and to produce an evolving three-dimensional (or in other
words, a 4D) map of the universe that is valid from before 380~kyr to
today, and out beyond 14 Gpc. The exercise is not purely cartographic as
it is essential that we use the secure physical inferences that have
been drawn about the early universe in making this map.

The first stage of our proposed program uses 2D CMB observations alone
...
surface. In the second stage, we augment CMB data with existing 3D
...
The fourth and final stage is an investigation of how far it is
...
Most quantitative cosmology derives from CMB observations. The
...
Most of the use of surveys has been for drawing statistical inferences
...
the resulting 3D map. A good example is the SDSS/BOSS
...
scales can be estimated but with inferior precision.
...
is comparable with the precision with which the fractional density
